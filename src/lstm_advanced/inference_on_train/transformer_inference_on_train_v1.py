...
# ============================================================
def main():
    print("=== Generating Transformer TRAIN Embeddings (BATCHED) ===")
    ...
    # ------------------------------------------------------------
    # 2) LOAD TRAINED TRANSFORMER MODEL
    # ------------------------------------------------------------
    ckpt_path = "results/lstm_advanced/transformer_baseline.pt"
    print("[INFO] Loading Transformer model:", ckpt_path)
    ...
    with torch.no_grad():
        ...
        for i in range(0, n, BATCH_SIZE):
            ...
            _, emb = model(batch_t)   # output embedding
    ...
    out_path = OUT_DIR / "transformer_embeddings_train.npy"
    np.save(out_path, final_emb)

    print(f"✅ Saved Transformer TRAIN embeddings → {out_path} | shape={final_emb.shape}")
