...
# ✅ FIXED LABEL MAPPING — verified correct order
LABELS = ["positive", "negative", "neutral"]
label2id = {"positive": 0, "negative": 1, "neutral": 2}
id2label = {0: "Positive", 1: "Negative", 2: "Neutral"}
print(f"[DEBUG] Label mapping: {label2id}")

BASE_MODEL = "ProsusAI/finbert"

# ✅ Tuned hyperparams for F1 boost
TRAIN_BS = 8
EVAL_BS  = 16
LR = 1.5e-5
EPOCHS = 6
...
# ✅ Enable Focal Loss for class imbalance
USE_FOCAL = True
FOCAL_GAMMA = 2.0
PRIOR_TARGET = "uniform"
...
trainer = ImbTrainer(model=model, args=args, train_dataset=train_ds,
                     eval_dataset=valid_ds, data_collator=data_collator,
...
def save_trainer_model(trainer, tokenizer, output_dir):
    trainer.model.model.config.label2id = label2id
    trainer.model.model.config.id2label = id2label
    trainer.save_model(str(output_dir))
    tokenizer.save_pretrained(str(output_dir))
    print(f"[✅ Model Saved with correct mapping] → {output_dir}")
    print(f"[DEBUG] Saved label2id = {trainer.model.model.config.label2id}")
...
def main():
    ...
    save_trainer_model(trainer, tokenizer, OUTPUT_DIR)
